...
class conv_2d_nchw_fchw_test(unittest.TestCase):
    ...
    def testBS32(self, atol, rtol):
        ...
        result = kernels.conv_2d_nchw_fchw(inputs, inputs_pad, weights, bias, [1, 1], [1, 1], [1, 1])

        # Tolerances are empirical and results are not expected to match exactly.
        ref = torch.nn.functional.conv2d(inputs, weights, bias=bias, stride=(1,1), padding=(1, 1), dilation=(1,1))
        print(result.shape)
        print(ref.shape)

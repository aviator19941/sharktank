...
class pooling_nchw_sum(CustomOp):
    ...
    * `qs`: `[N, K // 32, 32]`
    ...
    def generate(self, ksel: KernelSelection, kb: KernelBuilder):
        ...
        input_dim_sizes = 'x'.join(str(input_tensor_type.get_dim_size(i)) for i in range(input_tensor_type.rank))
        ...
        target_function_name = (
            f"sharktank_pooling_nchw_sum_{input_dim_sizes}_{weights[0]}_{weights[1]}_{strides[0]}_{strides[1]}_{padding[0]}_{padding[1]}_{dilations[0]}_{dilations[1]}_{dtype_str}"
        )
